...
func TestConfigLoadingWithEnv(t *testing.T) {
	...
	if cfg.PoolConfig.RedisPoolCfg.RedisURL != "redis://localhost:6379" {
...
func TestDefaultConfig(t *testing.T) {
	...
	if redisURL != "redis://arbitrary_username:password@8.8.8.8:6379/0" {
		t.Fatalf("expect redisURL '%s' but got '%s'\n", "redis://arbitrary_username:password@8.8.8.8:6379/0", redisURL)
	}
...
func setENV() {
	...
	os.Setenv("JOB_SERVICE_POOL_REDIS_URL", "redis://localhost:6379")
